...
def top_project_rdc():
    ...
    # top_rdc = df['rdc_mentions'].value_counts().head(10)
    fig, ax = plt.subplots()
...
def top_authors():
    author_series = df['authors'].dropna().str.split(',')
    authors_list = [author.strip() for sublist in author_series for author in sublist]
